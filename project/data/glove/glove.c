...
int initialize_from_file = 0; //0 or 1
char *initial_weight_file;
...
void *glove_thread(void *vid) {
    ...
    //fprintf(stderr, "Started glove threads %d with line per thread: %d\n",id,lines_per_thread[id]);

    for(a = 0; a < lines_per_thread[id]; a++) {
        
        //fprintf(stderr, "Started glove threads %d with line %d\n",id,a);

        fread(&cr, sizeof(CREC), 1, fin);

        //float val;
        //fscanf (fin, "%d %d %f\n", &(cr.word1), &(cr.word2), &(val));
        //cr.val = val;

        //fprintf(stderr, "Threads %d Read CREC with line %d\n",id,a);

        //if(fabs(cr.val) < 1E-10){
        //    fprintf(stderr, "CREC word1: %d\n",cr.word1);
        //    fprintf(stderr, "CREC word2: %d\n",cr.word2);
        //    fprintf(stderr, "CREC val: %f\n",cr.val);
        //}
        ...
        //fprintf(stderr, "loc l1: %d\n",l1);
        //fprintf(stderr, "loc l2: %d\n",l2);

        //fprintf(stderr, "Threads %d Done get location with line %d\n",id,a);
        
        /* Calculate cost, save diff for gradients */
        diff = 0;
        for(b = 0; b < vector_size; b++) diff += W[b + l1] * W[b + l2]; // dot product of word and context word vector
        //fprintf(stderr, "Threads %d Done dot product with line %d\n",id,a);
        ...
        //if(isnan(cost[id])){
        //    fprintf(stderr, "Line: %d Cost: %f T: %d CREC word1: %d word2: %d val: %f\n",a,cost[id],id,cr.word1,cr.word2,cr.val);
        //    fprintf(stderr, "loc l1: %d\n",l1);
        //    fprintf(stderr, "loc l2: %d\n",l2);
        //    fprintf(stderr, "vector_size + l2: %d\n",vector_size+l2);
        //    fprintf(stderr, "W size: %d\n",2 * vocab_size * vector_size);
        //    return;
        //}

        //fprintf(stderr, "Threads %d Done cost with line %d\n",id,a);
        ...
        //fprintf(stderr, "Threads %d Done update with line %d\n",id,a);

    }

    //fprintf(stderr, "Done loop threads %d\n",id,a);
    
    fclose(fin);

    //fprintf(stderr, "Done glove threads %d\n",id);
...
int save_params() {
    ...
    if(use_binary != 1) { // Save parameters in text file
        if(model == 2){
            char output_file1[MAX_STRING_LENGTH],output_file2[MAX_STRING_LENGTH];
            FILE *fout1, *fout2;

            sprintf(output_file1,"%s_word.txt",save_W_file);
            sprintf(output_file2,"%s_context.txt",save_W_file);
            ...
            fout1 = fopen(output_file1,"wb");
            fout2 = fopen(output_file2,"wb");
            if(fout1 == NULL) {fprintf(stderr, "Unable to open file %s.\n",output_file1); return 1;}
            if(fout2 == NULL) {fprintf(stderr, "Unable to open file %s.\n",output_file2); return 1;}
            ...
            for(a = 0; a < vocab_size; a++) {
                ...
                fprintf(fout1, "%s",word);
                fprintf(fout2, "%s",word);
                
                for(b = 0; b < vector_size; b++){
                    fprintf(fout1," %lf", W[a * (vector_size + 1) + b]);
                    fprintf(fout2," %lf", W[(vocab_size + a) * (vector_size + 1) + b]);
                }
...
int main(int argc, char **argv) {
    ...
    initial_weight_file = malloc(sizeof(char) * MAX_STRING_LENGTH);
    ...
    if ((i = find_arg((char *)"-initial-weight-file", argc, argv)) > 0){
        strcpy(initial_weight_file, argv[i + 1]);
