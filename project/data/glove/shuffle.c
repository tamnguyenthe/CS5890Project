...
long long array_size = 500000; // size of chunks to shuffle individually
...
int write_chunk(CREC *array, long size, FILE *fout) {
    ...
    for(i = 0; i < size; i++){
        ...
        //if(array[i].word1 == 15284){
        //fprintf(stderr, "Writing CREC word1: %d\n",array[i].word1);
        //fprintf(stderr, "Writing CREC word2: %d\n",array[i].word2);
        //fprintf(stderr, "Writing CREC val: %f\n",array[i].val);
        //}
    }
    return 0;
}
    ...
    while(1) { //Loop until EOF in all files
        ...
        for(j = 0; j < num; j++) {
            ...
            for(k = 0; k < array_size / num; k++){
                ...
                //float val;
                //fscanf (fid[j], "%d %d %f\n", &(array[i].word1), &(array[i].word2), &(val));
                //array[i].val = val;
                //if(current.word1 == 15284){
